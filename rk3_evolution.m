...
function psi = rk3_evolution(v_mat, psi_0, L, dt, kin_h_function, mass)
    ...
    for j = 1:n_t-1
        ...
        k1 = 1/(1i*h_bar)*H1*psi(:,j);
        %display(size(k1,1));
        %display(k1);
        
        H2 = diag(v_mat(:,2*j)) + T;
        k2 = 1/(1i*h_bar)*H2*(psi(:,j)+h/2*k1);
        
        H3 = diag(v_mat(:,2*j+1)) + T;
        k3 = 1/(1i*h_bar)*H3*(psi(:,j)-h*k1+2*h*k2);
        
        psi(:,j+1) = psi(:,j)+h/6*(k1 + 4*k2 + k3);
